...
class SimpleQMIXNetwork(nn.Module):
    def __init__(self, state_dim, num_agents, hidden_dim):
        ...
        self.q_layers = nn.ModuleList([nn.Linear(hidden_dim, 1) for _ in range(num_agents)])
        self.output_layer = nn.Linear(num_agents, 1)

    def forward(self, q_values):
        x = torch.relu(self.input_layer(q_values))
        agent_qs = [q_layer(x) for q_layer in self.q_layers]
        agent_qs = torch.cat(agent_qs, dim=1)
        q_total = self.output_layer(agent_qs)
        return q_total
...
if __name__ == "__main__":
    ...
    for episode in tqdm(range(qmix_episodes)):
        states = []
        total_rewards = np.zeros(region_num)

        for idx, env in enumerate(envs):
            state = env.reset(None)
            states.append(state)

        done_flags = [False for _ in range(region_num)]
        ...
        while not all(done_flags):
            ...
            for idx in range(region_num):
                ...
                q_values = local_q_networks[idx](x, edge_index, edge_attr, batch).detach()
        ...
        if all([len(buffer) >= batch_size for buffer in buffers]):
            ...
            for idx in range(region_num):
                ...
                q_eval = local_q_networks[idx](x_batch, edge_index_batch, edge_attr_batch, batch_batch).gather(1,
                                                                                                               action_batch.unsqueeze(
                                                                                                                   1)).squeeze(
                    1)
                next_q = target_local_q_networks[idx](next_x_batch, next_edge_index_batch, next_edge_attr_batch,
                                                      next_batch_batch).max(1)[0]
            ...
            q_total_eval = qmix_net(q_eval_stack)
            q_total_target = target_qmix_net(q_target_stack)
        ...
        if episode % 10 == 0:
            ...
            print(
                f"[Episode {episode}] QMIX Loss: {qmix_loss.item():.4f}, Avg Reward: {avg_reward:.4f}, Epsilon: {epsilon:.4f}")
